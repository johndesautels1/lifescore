 ...
 *
 * Cristiano HeyGen multi-scene video presentation of the winning city.
 * Displayed after the Court Order in the Judge tab flow.
 * Uses the same LCD display style as CourtOrderVideo.
 *
...
import type { CristianoScriptInput } from '../services/cristianoVideoService';
...
const GoToMyNewCity: React.FC<GoToMyNewCityProps> = ({
  ...
  const handleGenerate = async () => {
    ...
    const input: CristianoScriptInput = {
      winnerCity,
      loserCity,
      winnerScore,
      loserScore,
  ...
  const handleVideoError = useCallback((e: React.SyntheticEvent<HTMLVideoElement>) => {
    console.error('[GoToMyNewCity] Video load error:', e);
  ...
  const handleDownload = async () => {
    if (!videoUrl) return;
    const filename = `go-to-${winnerCity.replace(/[^a-z0-9]/gi, '-').toLowerCase()}-cristiano.mp4`;
    try {
  ...
  const handleShare = async () => {
    ...
    const shareData = {
      ...
      text: `Cristiano presents: Why ${winnerCity} is your new city with a LIFE SCORE of ${winnerScore.toFixed(1)}!`,
      url: videoUrl,
    };
    try {
    ...
    <FeatureGate feature="cristianoVideos" blurContent={true}>
      <div className="new-city-wrapper">
        <div className="new-city-container">
          ...
          {/* LCD Screen - Same display style as Court Order */}
                    ...
                    {isGenerating ? (
                      <div className="generating-state">
                        ...
                        <div className="lcd-status">
                          {status === 'processing'
                            ? `Cristiano is filming... ${progress}%`
                            : 'Preparing your city tour...'}
                        </div>
                      ...
                      <div className="idle-state">
                        ...
                        <div className="preview-text">
                          Let Cristiano show you your new city
                        </div>
                        <div className="preview-badge">
                          <span className="badge-icon">⚖️</span>
                          <span className="badge-text">PRESENTED BY CRISTIANO</span>
                        </div>
                      </div>
                    )}
                  </div>
          ...
          <div className="court-actions">
            ...
            {isReady && !isPlaying && (
              ...
              >
                <span className="btn-icon">{'\u25B6'}</span>
                <span className="btn-text">WATCH CRISTIANO'S TOUR</span>
              </button>
            )}
          </div>
