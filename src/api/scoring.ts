...
/**
 * Generate realistic demo scores for testing
 * IMPORTANT: This is ONLY for demo/testing purposes
 * Real scores MUST come from API with web search verification
 */
export function generateDemoScores(city: string, country: string): MetricScore[] {
  const scores: MetricScore[] = [];
  
  // Seed based on city name for consistent demo results
  ...
  for (const metric of ALL_METRICS) {
    // Generate a pseudo-random score based on city name
    const baseScore = random(offset++) * 100;
    
    // Add some variance based on country (European cities tend to score higher on certain metrics)
    let countryModifier = 0;
    if (['UK', 'Germany', 'Netherlands', 'Portugal', 'Spain', 'France', 'Ireland'].some(c => 
      country.toLowerCase().includes(c.toLowerCase())
    )) {
      // European cities get boost on certain categories
      if (['personal_freedom', 'transportation', 'policing_legal'].includes(metric.categoryId)) {
        countryModifier = 15;
      }
    }
    
    // US cities get boost on speech/business
    if (country.toLowerCase().includes('usa') || country.toLowerCase().includes('united states')) {
      if (['speech_lifestyle', 'business_work'].includes(metric.categoryId)) {
        countryModifier = 10;
      }
      // But lower on transportation typically
      if (metric.categoryId === 'transportation') {
        countryModifier = -20;
      }
    }

    const finalScore = Math.max(0, Math.min(100, baseScore + countryModifier));
    
    scores.push({
      metricId: metric.id,
      rawValue: finalScore,
      normalizedScore: Math.round(finalScore),
      confidence: random(offset++) > 0.2 ? 'high' : 'medium',
      notes: 'Demo data - not from web search'
    });
...
/**
 * Generate a complete demo comparison
 */
export function generateDemoComparison(
  city1: string,
  city2: string
): ComparisonResult {
  // Parse city strings (format: "City, State/Region, Country")
  const parseCity = (cityStr: string) => {
    const parts = cityStr.split(',').map(s => s.trim());
    return {
      city: parts[0] || cityStr,
  ...
  const city1Metrics = generateDemoScores(city1Info.city, city1Info.country);
  const city2Metrics = generateDemoScores(city2Info.city, city2Info.country);

  const city1Score = calculateCityScore(
    city1Info.city,
    city1Info.country,
    city1Metrics,
    city1Info.region
  ...
  const city2Score = calculateCityScore(
    city2Info.city,
    city2Info.country,
    city2Metrics,
    city2Info.region
  ...
  return createComparison(city1Score, city2Score);
}
