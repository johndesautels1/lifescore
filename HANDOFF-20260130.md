...
4. **Fixed Judge video Replicate settings** - Corrected SadTalker settings in generate-judge-video.ts to match working config:
   - `size_of_image: 256 → 512` (was wrong, now correct)
   - `use_enhancer: false → true` (was wrong, now correct)
...
6. **Full codebase audit completed** - Confirmed that `api/avatar/generate-judge-video.ts` is the ONLY file with actual SadTalker Replicate settings. All other files mentioning SadTalker are type definitions, cost calculators, or documentation.
