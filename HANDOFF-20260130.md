...
4. **Fixed Judge video GPU memory** - Reduced SadTalker settings in generate-judge-video.ts:
   - `size_of_image: 512 → 256`
   - `use_enhancer: true → false`
   - Fixed CHRISTIANO_IMAGE_URL from MP4 to PNG

5. **Fixed button text sizing** - JudgeTab.css action buttons (SAVE REPORT, DOWNLOAD PDF, DOWNLOAD VIDEO, FORWARD) - removed width constraints so text fits.
