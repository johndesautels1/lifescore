...
### 2. Cost Tracking Wiring - NEEDS VERIFICATION ⚠️
...
### 3. Gemini Business_Work Token Fix - FIXED ✅
**Commit:** `732b461`
...
- Tightened conciseness requirements for large categories:
  - "1-2 sentences (50 words)" → "1 sentence (25 words)"
  - "1-2 sources" → "1 source"
- Only affects Gemini, only for categories with 20+ metrics
- **File:** `api/evaluate.ts` lines 895-900

### 4. Judge Video Generation - COMPLETE OVERHAUL ✅
**Commits:** `fdef8ee`, `19c0dfd`

**Problem:** SadTalker requires audio URL, but code was passing script TEXT.

**Fixed flow:**
1. Script → ElevenLabs/OpenAI TTS → Audio buffer
2. Upload audio to Supabase Storage → Get public URL
...
- `ELEVENLABS_API_KEY` ✅
- `SUPABASE_URL` ✅
- `SUPABASE_SERVICE_KEY` ✅ (new sb_secret_... format)
...
- Open Cost Dashboard (💰 icon) - should show comparison with actual costs
- If no data: APIs may not be returning usage field - check network tab

### 2. Judge Video Generation
- Run enhanced comparison
- Go to Judge tab → Generate Report
- Click video button
...
2. **Christiano voice ID** - Using default Adam voice (`pNInz6obpgDQGcFmaJgB`). May want custom ElevenLabs voice. Env var: `ELEVENLABS_CHRISTIANO_VOICE_ID`
